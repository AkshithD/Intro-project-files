public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        //name
         String name = s.substring(s.indexOf("name:")+5, s.indexOf(";")); 
        //fuel info
         String fuel = s.substring(s.indexOf("type:")+5, s.indexOf(";"));
         double usage = s.substring(s.indexOf("mile/unit:")+10, s.indexOf(";"));
        //lease info
         double dueAtSigning = s.substring(s.indexOf("mile/unit:")+10, s.indexOf(";"));
         int leaseLength = s.substring(s.indexOf("length:")+7, s.indexOf(";"));
         double monthlyCost= s.substring(s.indexOf("monthly:")+8, s.indexOf(";"));
         int mileageAllowance=s.substring(s.indexOf("allowance:")+10, s.indexOf(";"));

         int charger=0; //need to make this not be there if the type is gas.
        if(s.contains("charger:")){
            Fuel (usage, charger = s.substring(s.indexOf("length:")+8, s.indexOf(";")));
        }else{
            Fuel (usage);
        }
        
         Vehicle (name, Fuel(), Lease(dueAtSigning,leaseLength,monthlyCost,mileageAllowance));
	    return null;
	}
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        for(int i=0; i<vehicles.length; i++){
            String type = getType ();
            if(type.equals("GAS")){
                co2PerUnit=CO2EMITTED_GASCOMBUSTION;
            }else{
                co2PerUnit=CO2EMITTED_ELECTRICITYCOMBUSTION;
            }
        //computing the CO2 emission
       computeCO2(getLeaseLength(), getUsage (), getMileageAllowance (), co2PerUnit );
       //Fuel costs
       computeFuelCost(getLeaseLength(), getUsage (), getMileageAllowance (), gasPrice);
       //lease costs
       computeLeaseCost(getDueAtSigning (), getLeaseLength(), getMonthlyCost ());
    	}
    }
